...
async function getInvitedLecturesByEmpId(empId) {
  let connection;
  try {
    connection = await oracledb.getConnection({
      user: process.env.DB_USER,
    ...
    const result = await connection.execute(
      `SELECT * FROM AC_INVITED_LECTURES WHERE AIL_EMPID = :empId`,
      { empId },
      { outFormat: oracledb.OUT_FORMAT_OBJECT }
    );
...
module.exports = { getEmpIdByDevice, getInvitedLecturesByEmpId };