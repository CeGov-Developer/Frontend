...
async function getWorkshopSeminarDataByEmpId(table, empCol, empId) {
  console.log(table, empCol, empId);
  let connection;
  try {
    connection = await oracledb.getConnection({
      user: process.env.DB_USER,
    ...
    const result = await connection.execute(
      `SELECT * FROM ${table} WHERE ${empCol} = :empId`,
      { empId },
      { outFormat: oracledb.OUT_FORMAT_OBJECT }
    );
    return result.rows;
