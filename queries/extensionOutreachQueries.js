...
async function getExtensionOutreachByEmpId(empId) {
  let connection;
  try {
    connection = await oracledb.getConnection({
      user: process.env.DB_USER,
    ...
    const result = await connection.execute(
      `SELECT * FROM AC_EXTENSION_OUTREACH WHERE AEO_EMPID = :empId`,
      { empId },
      { outFormat: oracledb.OUT_FORMAT_OBJECT }
    );
...
module.exports = { getEmpIdByDevice, getExtensionOutreachByEmpId };