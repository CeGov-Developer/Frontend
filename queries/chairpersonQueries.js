...
async function getChairpersons() {
  let connection;
  try {
    ...
    const result = await connection.execute(
      `SELECT EM_NAME, EM_DISPLAY_NAME,APD_CAMP_CODE,  APD_SEAT_NAME, EM_EMPLOYEE_ID
       ...
       WHERE AC_POSTING_DETAILS.APD_POSTING_CODE = 6 ORDER BY APD_CAMP_CODE, APD_DEPT_CODE
        `,
      [],
...
async function getChairpersonsContactDetails(campCodes) {
...
async function getChairpersonsImage(employeeIds) {
  ...
  module.exports = { getChairpersonsContactDetails, getChairpersonsImage, getChairpersons };